...
type FreshAppOptions = Partial<Pick<AppDeps, 'getReadinessChecks' | 'getReconciliationStatus'>>;
...
describe('health and readiness routes', () => {
  ...
  it('returns degraded readiness when a dependency check fails', async () => {
    ...
    expect(res.body.mode).toBe('partially_healthy');
    expect(res.body.degradedServices).toEqual(['soroban_rpc']);
...
describe('POST /api/orders/announce', () => {
  it('returns 201 for a valid order', async () => {
    const app = await freshApp();
    const res = await request(app).post('/api/orders/announce').send(BASE_ANNOUNCE);
  ...
  it('returns 400 for a missing field', async () => {
    ...
    const res = await request(app).post('/api/orders/announce').send(withoutHashlock);
  ...
  it('returns a structured 400 for a mismatched direction/chain combo', async () => {
    ...
    const res = await request(app)
      ...
      .send({
        ...BASE_ANNOUNCE,
        srcChain: 'solana',
        srcAddress: '11111111111111111111111111111111',
      });
    expect(res.status).toBe(400);
    expect(res.body.error).toBe('validation_error');
    expect(typeof res.body.message).toBe('string');
    expect(Array.isArray(res.body.details)).toBe(true);
    expect(res.body.details.some((d: { path: unknown[] }) => d.path.includes('srcChain'))).toBe(
      true
    );
  });
  ...
  it('returns 429 once the announce rate limit is exceeded', async () => {
    ...
    const makeHashlock = (i: number) => '0x' + i.toString(16).padStart(2, '0').repeat(32);
  ...
  it('X-Forwarded-For is ignored when no trusted proxy is configured', async () => {
    ...
    const makeHashlock = (i: number) => '0x' + (i + 100).toString(16).padStart(2, '0').repeat(32);
...
describe('POST /api/secrets/reveal', () => {
  ...
  it('returns 429 after the reveal rate limit (5/min) is exceeded', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      const res = await request(app).post('/api/secrets/reveal').send(payload);
      expect(res.status).toBe(404);
    }
...
describe('API key bypass', () => {
  ...
  it('a valid API key bypasses the announce rate limit', async () => {
    ...
    try {
      const app = await freshApp();
      const makeHashlock = (i: number) => '0x' + (i + 200).toString(16).padStart(2, '0').repeat(32);
...
describe('GET /api/orders/history', () => {
  ...
  it('accepts a valid Solana address (empty history is still 200)', async () => {
    const app = await freshApp();
    const res = await request(app).get('/api/orders/history').query({ address: VALID_SOLANA_ADDR });
    expect(res.status).toBe(200);

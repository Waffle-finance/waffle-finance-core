...
import { createReadinessChecks, deriveStartupPhase, type StartupPhase } from '../src/readiness.js';
import { evaluateDependencyHealth } from '../src/degraded-mode.js';
...
describe('evaluateDependencyHealth', () => {
  it('reports partially_healthy when a chain dependency degrades but the database remains healthy', () => {
    ...
    expect(report.degradedServices).toEqual(['ethereum_rpc']);
  });

  it('reports degraded when the database dependency is unavailable', () => {
    const report = evaluateDependencyHealth([
    ...
    expect(report.degradedServices).toEqual(['database']);
  });

  it('reports healthy when every required dependency is healthy', () => {
    const report = evaluateDependencyHealth(makeChecks());
